...
class Menu {
    ...
    public function __construct() {
        $this->list = &$_SESSION['daftarProduk']; // pakai reference ke session
    }
    ...
    public function tambahProduk($id, $nama, $harga, $stok, $fotoFile) {
        ...
        if ($fotoFile && $fotoFile['error'] == 0) {
            $foto = $this->handleFileUpload($fotoFile);
            if (empty($foto)) {
        ...
        $this->list[] = $produk->toArray();
        echo "<script>alert('Produk berhasil ditambahkan!');</script>";
    }

    public function updateData($id, $nama, $harga, $stok, $fotoFile) {
        ...
        if (!empty($nama))  $this->list[$index]['nama']  = $nama;
        if (!empty($harga)) $this->list[$index]['harga'] = $harga;
        if (!empty($stok))  $this->list[$index]['stok']  = $stok;

        if ($fotoFile && $fotoFile['error'] == 0) {
            $foto = $this->handleFileUpload($fotoFile);
            if (!empty($foto)) {
                $this->list[$index]['foto'] = $foto;
            }
        }
    ...
    public function tampilProduk($search = '') {
        $list = $this->list;
        if (!empty($search)) {
            $search = strtolower($search);
            $list = array_filter($this->list, function($item) use ($search) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['create'])) {
        $menu->tambahProduk($_POST['id'], $_POST['nama'], $_POST['harga'], $_POST['stok'], $_FILES['foto']);
    } elseif (isset($_POST['update'])) {
        $menu->updateData($_POST['id'], $_POST['nama'], $_POST['harga'], $_POST['stok'], $_FILES['foto']);
    } elseif (isset($_POST['delete'])) {
        $menu->hapusData($_POST['id']);
    }
}
